...
from .storage import _eq, create_group, delete_group, find_group, group_by_pin, list_versions, load, load_groups, read_log, save, save_groups
...
def _admin_api(environ, start_response, parts, method, body):
    ...
    with LOCK:
        ...
        if parts == ["api", "admin", "groups"] and method == "POST":
            gid = create_group(text(body.get("name"), 60), text(body.get("pin"), 32))
        ...
        if len(parts) == 5 and parts[:3] == ["api", "admin", "group"] and parts[4] == "delete" and method == "POST":
            gid = parts[3]
            if not find_group(gid):
                return _json(start_response, {"error": "Ni skupine"}, "404 Not Found")
            delete_group(gid)
